...
class LaneATT(nn.Module):
    ...
    def forward(self, x, conf_threshold=None, nms_thres=0, nms_topk=3000):
        ...
        attention_matrix = torch.eye(attention.shape[1], device=x.device).repeat(
            x.shape[0], 1, 1
        )
        ...
        proposals_list = self.nms(
            reg_proposals, attention_matrix, nms_thres, nms_topk, conf_threshold
        )
    ...
    def nms(
        ...
        for proposals, attention_matrix in zip(batch_proposals, batch_attention_matrix):
            ...
            with torch.no_grad():
                ...
                if proposals.shape[0] == 0:
                    proposals_list.append(
                        (proposals[[]], self.anchors[[]], attention_matrix[[]], None)
                    )
                    continue
            ...
            proposals_list.append(
                (proposals, self.anchors[keep], attention_matrix, anchor_inds)
            )
        ...
        for (proposals, anchors, _, _), target in zip(proposals_list, targets):
            ...
            with torch.no_grad():
                ...
                invalid_offsets_mask = torch.zeros(
                    (num_positives, 1 + self.n_offsets + 1), dtype=torch.int
                )  # length + S + pad
...
def get_backbone(backbone, pretrained=False):
    if backbone == "resnet122":
        ...
        backbone = torch.nn.Sequential(
            *list(resnet34(pretrained=pretrained).children())[:-2]
        )
        fmap_c = 512
        stride = 32
